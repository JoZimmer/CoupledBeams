...
# OLD 
def plot_block_stats (given_series, abs_block_size, block_index, time_series):
    m_tot = np.mean(given_series)
    s_tot = np.std(given_series)
    sk_tot = sps.skew(given_series)

    n = len(block_index)
    while n % 4 != 0:
        n += 1

    fig, axs = plt.subplots(4, int(n/4))
    fig.suptitle(get_title(given_series))
    axs = axs.ravel()

    m_i, s_i, sk_i, anderson = get_block_stats(given_series, abs_block_size, block_index)
    ...
    for i in range(len(block_index)-1):
        ...
        x,y = get_pdf_normal_scipy(sub)
        axs[i].plot(x,y, label = str(round(m_i[i],2)) + '\n' + str(round(s_i[i],2)) +  '\n' + str(round(sk_i[i],2)))
        axs[i].hist(sub, bins= 100, density = True, color = 'lightgray')
        axs[i].legend()
    ...
    for i, m in enumerate(m_i):
        if abs((abs(m_tot) - abs(m))) > 0.01*m_tot:
            txt_m += 'B' + str(i+1) + ': ' + str(round(m,2)) +  '\n'
        d0 = 0.01 * s_tot
        d1 = s_tot - s_i[i]
        if abs(abs(s_tot) - abs(s_i[i])) > 0.01 * s_tot:
            txt_s += 'B' + str(i+1) + ': ' + str(round(s_i[i],2)) +  '\n'
        if abs((abs(sk_tot) - abs(sk_i[i]))) > 0.01 * abs(sk_tot):
            if n_sk > 9:
                pass
            else:
                n_sk += 1
                txt_sk += 'B' + str(i+1) + ': ' + str(round(sk_i[i],2)) +  '\n'

    b_i = np.arange(1, len(m_i) + 1)
    
    fig_b = plt.figure('block_stats')

    ax_m = fig_b.add_subplot(311)
    ax_m.hlines(m_tot, 0, b_i[-1], label = 'mean_total ' + str(round(m_tot,2)))
    ax_m.scatter(b_i, m_i)

    ax_m.set_xlabel('block number')
    ax_m.set_ylabel('mean')
    ax_m.legend(loc= 'lower left')
    txt_box_m = AnchoredText(txt_m, loc = 2)
    ax_m.add_artist(txt_box_m)

    ax_s = fig_b.add_subplot(312)
    ax_s.hlines(s_tot, 0, b_i[-1], label = 'std_total '+ str(round(s_tot,2)))
    ax_s.scatter(b_i, s_i)

    ax_s.set_xlabel('block number')
    ax_s.set_ylabel('std')
    ax_s.legend(loc= 'lower left')

    txt_box_s = AnchoredText(txt_s, loc = 2)
    ax_s.add_artist(txt_box_s)

    ax_sk = fig_b.add_subplot(313)
    ax_sk.hlines(sk_tot, 0, b_i[-1], label = 'skew_total '+ str(round(sk_tot,2)))
    ax_sk.scatter(b_i, sk_i)

    ax_sk.set_xlabel('block number')
    ax_sk.set_ylabel('skewness')
    ax_sk.legend(loc= 'lower left')

    txt_box_sk = AnchoredText(txt_sk, loc = 2)
    ax_sk.add_artist(txt_box_sk)

# NEW -> use for block size compare
