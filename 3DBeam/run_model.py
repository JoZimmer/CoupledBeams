...
einwirkungs_df_typ = pd.DataFrame(columns=df_einwirkungs_header_typ)
    ...
    for typ in lasten_dicts_typ[QS_label][qs.nabenhöhe]:
        if typ == 'bauzustand':
            ...
            for segment in lasten_dicts_typ[QS_label][qs.nabenhöhe][typ]:
                filename = 'K-IEA@maxFx_' + 'W-v' +str(basis_windgeschwindigkeit) + 'cd' + str(qs.cd) + '_D-' + typ + '_' + segment
...
for segment in schnittgrößen_design['bauzustand'][QS_label][nabenhöhe]:
    if include_bauzustand_sgr:
        bauzustands_df.loc[:,(nabenhöhe, QS_label, segment, 'M [MNm]')] = np.around(schnittgrößen_design['bauzustand'][QS_label][nabenhöhe][segment]['g']* utils.unit_conversion('Nm', 'MNm') ,r_2)
